...
BASE_MODEL_ARG="--model-name-or-path ${MODEL_NAME_OR_PATH}"
...
python -m dp.train_nmt \
  ...
  --out artifacts/nmt/byt5_reverse_evacun_pre \
  ${BASE_MODEL_ARG} \
  --src-col tgt_sent --tgt-col src_sent \
  --post-eval-mode none

# 2) aligned のみで逆学習を仕上げ
python -m dp.train_nmt \
  --config "${NMT_CONFIG}" \
  --data-dir "${COMP_DATA_DIR}" \
  --train artifacts/aligned/aligned_train.parquet \
  --variant C --drop-flagged \
  --out artifacts/nmt/byt5_reverse_evacun \
  --model-name-or-path artifacts/nmt/byt5_reverse_evacun_pre \
  --src-col tgt_sent --tgt-col src_sent \
...
SRC_DIR="artifacts/nmt/byt5_reverse_evacun"
...
# Drive に保存したディレクトリ
SRC_DIR="/content/drive/MyDrive/translate-akkadian/artifacts/nmt/byt5_reverse_evacun"

# /content 側の復元先
DST_DIR="artifacts/nmt/byt5_reverse_evacun"
...
SRC_DIR="${REPO_DIR}/artifacts/nmt/byt5_small_colab_v1"
mkdir -p "${DRIVE_DIR}"
...
# PN/GN のみを抽出した軽量 lexicon（あなたの配置先に合わせて変更）
LEXICON_PATH="data/OA_Lexicon_eBL_PN_GN_no_word_overlap.csv"
FWD_CKPT_DIR="/content/models/byt5_small_colab_v1"

mkdir -p artifacts/tm/fold0
...
python -m dp.tm_index \
  ...
  --strip-lex-from-src \
  --out artifacts/tm/fold0/index.joblib

python -m dp.tm_generate_candidates \
  --index artifacts/tm/fold0/index.joblib \
  --queries artifacts/oracle/val_for_oracle.csv \
  --id-col id --src-col src \
  --config "${NMT_CONFIG}" \
  --fwd-ckpt "${FWD_CKPT_DIR}" \
  --lexicon "${LEXICON_PATH}" \
  --strip-lex-from-src \
  --topk 50 --dedupe --exclude-exact-src \
  --out artifacts/oracle/cands_tmpp_top50.csv
```
...
python -m dp.infer_editor_nbest \
  --config "${NMT_CONFIG}" \
  --ckpt artifacts/editor_model/byt5_editor_v1 \
  --fwd-ckpt "${FWD_CKPT}" \
