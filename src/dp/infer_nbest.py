"""Generate n-best predictions using TF-IDF retrieval baseline."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Infer n-best predictions with TF-IDF retrieval baseline.")
    ...
    parser.add_argument("--out", required=True, help="Output nbest CSV path.")
    ...
    rows = []
    for row_id, cand_idxs, cand_dists in zip(test_df[id_col], indices, distances):
        for rank, (idx, dist) in enumerate(zip(cand_idxs, cand_dists), start=1):
            sim = float(1.0 - dist)
            rows.append(
                {
                    "id": row_id,
