...
python -m dp.train_nmt   --config "${NMT_CONFIG}"   --data-dir "${COMP_DATA_DIR}"   --train artifacts/aligned/aligned_train.parquet   --variant C --drop-flagged   --out artifacts/nmt/byt5_small_dryrun   --max-train-rows 200 --max-val-rows 50   --post-eval-mode quick
```
...
source /content/colab_env.sh
cd "$REPO_DIR"
python -m dp.align_train --config configs/align.yaml   --data-dir "${COMP_DATA_DIR}"   --variant C   --drop-flagged    --use-oare-sentences
```
...
python -m dp.train_nmt   --config "${NMT_CONFIG}"   --data-dir "${COMP_DATA_DIR}"   --train artifacts/aligned/aligned_train.parquet   --variant C --drop-flagged   --out artifacts/nmt/byt5_small_colab   ${MODEL_ARG}   --post-eval-mode full
```
...
python -m dp.train_nmt   --config "${NMT_CONFIG}"   --data-dir "${COMP_DATA_DIR}"   --train artifacts/aligned/aligned_train.parquet   --variant C --drop-flagged   --out artifacts/nmt/byt5_small_colab_bf16   ${MODEL_ARG}   --post-eval-mode quick
```
