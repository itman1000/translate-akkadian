...
## 0-B. （オフライン用）ByT5（base/large）のモデルDataset
Kaggle の再実行ではインターネットが使えない場合があるため、**使いたいサイズのモデルを事前に Dataset 化**します。
...
`models/byt5-base` / `models/byt5-large` を zip 化して Kaggle Dataset にアップロードし、Notebook に追加してください。  
（Dataset 名の例: `byt5-base-model`, `byt5-large-model`）
...
- （任意）ByT5（base/large）のモデル Dataset
...
cd "$REPO_DIR"
# ★モデル指定（base/large）:
# - 基本はオフライン（Kaggle再実行）想定なので、ByT5 のモデルDatasetを追加して MODEL_DIR を存在させるのが安全です。
# - ネットが使える場合は Hugging Face（google/byt5-<variant>）にフォールバックします。
...
- ByT5-large の場合は `gradient_checkpointing` / `fp16`（T4/P100）/ `bf16`（A100 等）も検討する
...
- Kaggle の提出再実行はオフラインになりがちなので、`MODEL_DIR`（ByT5-base/large）を必ず Data に追加してください
